...
geturls_text = geturls_opener.open("http://theahl.com/stats/schedule.php?date="+getforyear+"-"+getformonth+"-"+getforday);
...
pre_get_todays_games = re.escape("<a href=\"game-summary.php?game_id=")+"([0-9]+)"+re.escape("\">Game Summary</a>");
...
while(cur_todays_games<num_todays_games):
 ...
 urllib2.install_opener(geturls_opener);
 geturls_text = geturls_opener.open("http://theahl.com/stats/game-summary.php?game_id="+get_todays_games[cur_todays_games]);
 if(geturls_text.info().get("Content-Encoding")=="gzip" or geturls_text.info().get("Content-Encoding")=="deflate"):
  strbuf = StringIO.StringIO(geturls_text.read());
  gzstrbuf = gzip.GzipFile(fileobj=strbuf);
