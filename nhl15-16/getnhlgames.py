...
pre_get_todays_games = re.escape("<a class=\"btn\" shape=\"rect\" href=\"http://www.nhl.com/gamecenter/en/recap?id=")+"([0-9]+)"+re.escape("\"><span>RECAP›</span></a>");
...
if(num_todays_games>0):
 print("print(\"Inserting \"+leaguename+\" Game Data From "+getformonth.lstrip('0')+"/"+getforday.lstrip('0')+"/"+getforcuryear+".\\n\");");
while(cur_todays_games<num_todays_games):
 ...
 numpers = 3;
 pre_curgame_score_text = re.escape("<span style=\"display: none;\" class=\"prd")+"([0-9]+)"+re.escape("\">")+"([0-9]+)"+re.escape("</span>");
 curgame_score_text = re.findall(pre_curgame_score_text, curgame_text);
 pre_curgame_otscore_text = re.escape("<span style=\"display: none;\" class=\"ot\">")+"([0-9]+)"+re.escape("</span>");
 curgame_otscore_text = re.findall(pre_curgame_otscore_text, curgame_text);
 if(len(curgame_otscore_text)==2):
  numpers = 4;
 pre_curgame_soscore_text = re.escape("<span style=\"display: none;\" class=\"details\">")+"([0-9]+)"+re.escape(" - ")+"([0-9]+)"+re.escape("</span>");
 ...
 if(len(curgame_soscore_text)==2 and (int(curgame_soscore_text[0][0])>0 or int(curgame_soscore_text[1][0])>0)):
  numpers = 5;
 pre_team_text = re.escape("<img title=\"")+"([a-zA-Z\. ]+)"+re.escape("\" alt=\"")+"([a-zA-Z\. ]+)"+re.escape("\" class=\"team-logo  \" src=\"");
 team_text  = re.findall(pre_team_text, unicodedata.normalize('NFKD', curgame_text.decode("utf-8")).encode('ASCII', 'ignore'));
 pre_team_shots = re.escape("colspan=\"1\" rowspan=\"1\" class=\"aShots\">")+"([0-9]+)"+re.escape("</td>");
 away_team_shots = re.findall(pre_team_shots, curgame_text);
 pre_team_shots = re.escape("colspan=\"1\" rowspan=\"1\" class=\"hShots\">")+"([0-9]+)"+re.escape("</td>");
