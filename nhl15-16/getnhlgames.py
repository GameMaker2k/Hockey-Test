...
geturls_text = geturls_opener.open("http://www.nhl.com/ice/schedulebyday.htm?date=" +
                                   getformonth+"/"+getforday+"/"+getforcuryear+"&season="+getforyear+getendyear)
...
pre_get_todays_games = re.escape(
    "<a class=\"btn\" shape=\"rect\" href=\"http://www.nhl.com/gamecenter/en/recap?id=")+"([0-9]+)"+re.escape("\"><span>RECAP›</span></a>")
...
if(num_todays_games > 0):
    print("print(\"Inserting \"+leaguename+\" Game Data From " +
          getformonth.lstrip('0')+"/"+getforday.lstrip('0')+"/"+getforcuryear+".\\n\");")
while(cur_todays_games < num_todays_games):
    ...
    geturls_opener.addheaders = [("Referer", "http://www.nhl.com/ice/schedulebyday.htm?date="+getformonth+"/"+getforday+"/"+getforcuryear+"&season="+getforyear+getendyear), ("User-Agent", useragent), ("Accept-Encoding", "gzip, deflate"),
    ...
    geturls_text = geturls_opener.open(
        "http://www.nhl.com/gamecenter/en/boxscore?id="+get_todays_games[cur_todays_games])
    if(geturls_text.info().get("Content-Encoding") == "gzip" or geturls_text.info().get("Content-Encoding") == "deflate"):
        strbuf = StringIO.StringIO(geturls_text.read())
        gzstrbuf = gzip.GzipFile(fileobj=strbuf)
    ...
    numpers = 3
    pre_curgame_score_text = re.escape(
        "<span style=\"display: none;\" class=\"prd")+"([0-9]+)"+re.escape("\">")+"([0-9]+)"+re.escape("</span>")
    ...
    if(len(curgame_otscore_text) == 2):
        numpers = 4
    pre_curgame_soscore_text = re.escape(
        "<span style=\"display: none;\" class=\"details\">")+"([0-9]+)"+re.escape(" - ")+"([0-9]+)"+re.escape("</span>")
    curgame_soscore_text = re.findall(pre_curgame_soscore_text, curgame_text)
    if(len(curgame_soscore_text) == 2 and (int(curgame_soscore_text[0][0]) > 0 or int(curgame_soscore_text[1][0]) > 0)):
        numpers = 5
    ...
    if(numpers == 3):
        print("MakeHockeyGame((sqlcur, sqlcon), "+getforcuryear+newgetformonth+newgetforday+", \""+team_text[1][0]+"\", \""+team_text[0][0]+"\", \""+curgame_score_text[3][1]+":"+curgame_score_text[0][1]+","+curgame_score_text[4][1]+":"+curgame_score_text[
              1][1]+","+curgame_score_text[5][1]+":"+curgame_score_text[2][1]+"\", \""+home_team_shots[0]+":"+away_team_shots[0]+","+home_team_shots[1]+":"+away_team_shots[1]+","+home_team_shots[2]+":"+away_team_shots[2]+"\", 0, False);")
    if(numpers == 4):
        print("MakeHockeyGame((sqlcur, sqlcon), "+getforcuryear+newgetformonth+newgetforday+", \""+team_text[1][0]+"\", \""+team_text[0][0]+"\", \""+curgame_score_text[3][1]+":"+curgame_score_text[0][1]+","+curgame_score_text[4][1]+":"+curgame_score_text[1][1]+","+curgame_score_text[5][1]+":" +
              curgame_score_text[2][1]+","+curgame_otscore_text[1]+":"+curgame_otscore_text[0]+"\", \""+home_team_shots[0]+":"+away_team_shots[0]+","+home_team_shots[1]+":"+away_team_shots[1]+","+home_team_shots[2]+":"+away_team_shots[2]+","+home_team_shots[3]+":"+away_team_shots[3]+"\", 0, False);")
    if(numpers == 5):
        print("MakeHockeyGame((sqlcur, sqlcon), "+getforcuryear+newgetformonth+newgetforday+", \""+team_text[1][0]+"\", \""+team_text[0][0]+"\", \""+curgame_score_text[3][1]+":"+curgame_score_text[0][1]+","+curgame_score_text[4][1]+":"+curgame_score_text[1][1]+","+curgame_score_text[5][1]+":"+curgame_score_text[2][1]+"," +
              curgame_otscore_text[1]+":"+curgame_otscore_text[0]+","+curgame_soscore_text[1][0]+":"+curgame_soscore_text[0][0]+"\", \""+home_team_shots[0]+":"+away_team_shots[0]+","+home_team_shots[1]+":"+away_team_shots[1]+","+home_team_shots[2]+":"+away_team_shots[2]+","+home_team_shots[3]+":"+away_team_shots[3]+"\", 0, False);")
